window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var WelcomeNoticeStore = class {
			...
			*/
			...
			async load() {
				...
				try {
					...
					const view = (response?.result?.value?.namespaces ?? response?.value?.namespaces ?? []).find((candidate) => candidate.ns === WELCOME_NOTICE_SETTINGS_NAMESPACE);
					if (generation !== this.generation) return;
					this.store.update((state) => {
						state.status = "ready";
						state.acknowledged = view !== void 0 && acknowledgementOf(view) === "2026-08-13.1";
						state.error = null;
					});
				} catch {
					if (generation !== this.generation) return;
			...
			*/
			async acknowledge() {
				...
				try {
					try {
						await this.api.settings.mutate(WELCOME_NOTICE_SETTINGS_NAMESPACE, [{
							op: "set",
		...
		*/
		function createApiAdapter(ctx) {
			const connApi = ctx.get("connection")?.api;
			...
			return {
				settings: {
					describe: async (req) => {
						...
						if (connApi?.settings?.describe) return connApi.settings.describe(req ?? {});
						throw new Error("settings service is not available");
					},
					mutate: async (args, ...rest) => {
						let ns;
						let ops;
						...
						if (remote?.settings?.mutate) {
							...
							let error = res.error;
							if (!res.ok && res.error) {
								const code = res.error.code === "settings/conflict" || res.error.code === "settings-conflict" ? "settings-conflict" : res.error.code;
								error = {
									...res.error,
									code
								};
						...
						if (connApi?.settings?.mutate) return connApi.settings.mutate({
							ns,
							ops,
							expectedRevision
						});
				...
				credentials: {
					describe: async (args) => {
						...
						if (connApi?.credentials?.describe) return connApi.credentials.describe({ refs });
					...
					set: async (args, val) => {
						...
						if (connApi?.credentials?.set) return connApi.credentials.set({
							ref,
							value
						});
						throw new Error("credentials service is not available");
					...
					unset: async (args) => {
						...
						if (connApi?.credentials?.unset) return connApi.credentials.unset({ ref });
						throw new Error("credentials service is not available");
					}
				},
				llm: {
					providers: async (req) => {
						...
						if (connApi?.llm?.providers) return connApi.llm.providers(req ?? {});
						throw new Error("llm service is not available");
					},
					discoverModels: async (args, maybeRequest) => {
						let settingsNs;
						let request;
						if (typeof args === "string") {
							settingsNs = args;
						...
						if (connApi?.llm?.discoverModels) return connApi.llm.discoverModels(args, maybeRequest);
						throw new Error("llm service is not available");
					}
				}
			};
		...
		*/
		function apply(ctx) {
			...
			const welcomeController = new WelcomeNoticeStore(api, connection?.isLoopback ?? true ? "host" : "memory");
